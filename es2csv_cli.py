...
description:     Command line utility, written in Python, for querying Elasticsearch in Lucene query syntax or Query DSL
                    syntax and exporting result as documents into a CSV file.

usage:
es2csv -q '*' -i _all -e -o ~/file.csv -k -m 100
...
es2csv -q '{"query": {"match_all": {}} -r -u http://login:password@kibana.com:6666/es/ -o ~/file.csv.
"""
import argparse
import sys
...
def main() -> None:
    ...
    p.add_argument(
        ...
        nargs="+",
        metavar="FIELDS",
        help="List of <field>:<direction> pairs to sort on. Default is %(default)s.",
    )
    p.add_argument(
        "-d",
        "--delimiter",
        ...
        default=",",
        type=str,
        help='Delimiter to use in CSV file. Default is "%(default)s".',
    )
    p.add_argument(
        "-m",
        "--max",
    ...
    p.add_argument(
        ...
        dest="verify_certs",
        action="store_true",
        help="Verify SSL certificates. Default is %(default)s.",
    )
    ...
    p.add_argument(
        "-v", "--version", action="version", version="%(prog)s " + __version__, help="Show version and exit."
    )
